...
function fnGetProcessAccounts()
{
	...
	$strApiKey = "keyOhmYh5N0z83L5F";
	$airtable_url = 'https://api.airtable.com/v0/' . $base . '/' . $table;
	$url = 'https://api.airtable.com/v0/' . $base . '/' . $table."?maxRecords=5&view=".rawurlencode("opportunity_not_processed");
				...
				if($arrAccount)
				{
					...
					if(is_array($arrOpportunityDetail) && (count($arrOpportunityDetail)>0))
					{
						...
						if(is_array($arrAccountDetailSF['records']) && (count($arrAccountDetailSF['records'])>0))
						{
							...
							$IsToBeInserted = fnCheckIfOppHistoryToBeInserted($arrAccountDetailSF['records']);
							if($IsToBeInserted)
							{
								if($IsToBeInserted == "1")
								{
...
function fnCheckIfOppHistoryToBeInserted($arrAccountHistory = array())
{
	if(is_array($arrAccountHistory) && (count($arrAccountHistory)>0))
	{
		...
		$strApiKey = "keyOhmYh5N0z83L5F";
		$url = 'https://api.airtable.com/v0/' . $base . '/' . $table."?maxRecords=1&view=".rawurlencode("latestoppfirst");
