...
function fnGetProcessAccounts()
{
	...
	$strApiKey = "keyOhmYh5N0z83L5F";
	$airtable_url = 'https://api.airtable.com/v0/' . $base . '/' . $table;
	$url = 'https://api.airtable.com/v0/' . $base . '/' . $table."?maxRecords=5&view=".rawurlencode("Unmapped Attendees");
...
function fnCheckIfContactHistoryToBeInserted($arrAccountHistory = array())
{
	if(is_array($arrAccountHistory) && (count($arrAccountHistory)>0))
	{
		...
		$strApiKey = "keyOhmYh5N0z83L5F";
		$url = 'https://api.airtable.com/v0/' . $base . '/' . $table."?maxRecords=1&view=".rawurlencode("latestahisfirst");
...
function fnGetContactDetailFromSf($instance_url, $access_token,$strEmail = "")
{
	if($strEmail)
	{
		...
		$query = "SELECT Name, Id, Email, Title, MailingCity, AccountId from Contact WHERE Email = '".$strEmail."' LIMIT 1";
