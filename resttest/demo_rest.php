...
<html>
    ...
    <body>
        <tt>
			...
			$strCon = mysql_connect("localhost","root","");
			if(!$strCon)
			{
				echo mysql_error();
				exit;
			...
			{
				$strDb = mysql_select_db("attent",$strCon);
				if(!$strDb)
				{
					echo mysql_error();
					exit;
				...
				else
				{
					...
					{
						$strUpdateQuery = "UPDATE salesuser SET user_token='".$access_token."', email='".$instance_url."' , salesuseraccesstoken = '".json_encode($_SESSION['arraccess'])."' WHERE salesuser_id='1'";
					...
					else
					{
						...
						if(is_array($arrRowsData) && (count($arrRowsData)>0))
						{
							...
							$arrAccessTok = json_decode($arrRowsData['salesuseraccesstoken'],true);
						}
					}
				}
			}
